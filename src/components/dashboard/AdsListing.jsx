...
const AdsListing = () => {
  ...
  {allAdds && allAdds.length ? allAdds.map((ad) => {
    return (
      <div key={`ad_listing_${ad.id}`} className="border p-4 rounded-lg shadow flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex gap-4 flex-1">
          <img src={ad?.image ?? ""} alt="Ad Image" className="rounded-lg w-24 h-24 object-cover" />
          <div>
